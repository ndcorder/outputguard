...
class TestSchemaValidation:
    @pytest.mark.parametrize("text,schema,should_pass", [
        ('{"name":"A","age":1}', _OBJ_NAME_AGE, True),
        ('{"name":"A"}', _OBJ_NAME_AGE, False),
        ("[1,2,3]", _ARR_INT, True),
        ('[1,"two",3]', _ARR_INT, False),
        ('{"x": 1.5}', _OBJ_X_NUM, True),
        ('{"x": 15}', {"type": "object", "properties": {"x": {"type": "number", "maximum": 10}}}, False),
...
class TestEmptyAndGarbage:
    @pytest.mark.parametrize("text", [
        "", "   ", "\n\n", "\t", "not json at all", "random gibberish xyz",
        "SELECT * FROM users", "<html><body>Hello</body></html>",
        "# Markdown heading", "def foo(): pass", "console.log('hi')",
        "---\ntitle: yaml\n---",
    ])
    ...
    @pytest.mark.parametrize("text,expected", [
        ...
        ("42", 42), ('"just a string"', "just a string"),
    ])
